...
export default function ManageArtworks() {
  ...
  const reject = async () => {
    ...
    if (!rejectReason.trim()) {
      toast.error("Tuliskan alasan yang spesifik terhadap karya ini");
      return;
    }

    setRejectLoading(true);
    try {
      await api.put(`/admin/artworks/${rejectTarget.id}/reject`, {
        message: rejectReason.trim(),
      });
      toast.success("Karya ditolak dan email terkirim");
      setArtworks((prev) =>
        prev.map((a) =>
      ...
      toast.error(error.response?.data?.message || "Gagal menolak karya");
    } finally {
              ...
              <div key={a.id} className="card p-4 flex items-center gap-4">
                ...
                <div className="flex items-center gap-2 shrink-0 flex-wrap justify-end">
                  ...
                  {a.status !== "verified" && (
                    <button
                      onClick={() => verify(a.id)}
                      className="bg-green-600 hover:bg-green-700 text-white text-xs py-1.5 px-3 rounded-lg transition-all"
                    >
                      ✅ Verifikasi
                    </button>
                  )}
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <div className="w-full max-w-lg rounded-3xl bg-slate-950 border border-slate-700 shadow-2xl overflow-hidden">
            <div className="border-b border-slate-700 px-6 py-5">
              <h2 className="text-lg font-semibold text-white">Tolak Karya</h2>
            </div>
            <div className="p-6 space-y-4">
              ...
              <textarea
                ...
                placeholder="Tuliskan alasan yang spesifik terhadap karya ini"
                className="w-full rounded-2xl border border-slate-700 bg-slate-900 px-4 py-3 text-sm text-white outline-none transition focus:border-primary-500"
              />
            </div>
