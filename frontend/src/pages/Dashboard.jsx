...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (user?.role !== 'mahasiswa') { setLoading(false); return }
    api.get('/artworks?my=1&per_page=20')
      ...
      <div className="max-w-6xl mx-auto px-4 py-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-10">
          ...
          <div className="card p-5 col-span-1">
            ...
            {user?.nim && <p className="text-xs text-white/40">NIM: {user.nim}</p>}
            {user?.program_studi && <p className="text-xs text-primary-400 mt-1">{user.program_studi.nama_prodi}</p>}
            <Link to="/profile" className="btn-secondary w-full text-center text-sm py-2 mt-4 block">
          ...
          {user?.role === 'mahasiswa' && (
            <>
              <div className="card p-5 flex flex-col items-center justify-center text-center">
                <span className="text-4xl font-bold text-primary-400">{myArtworks.length}</span>
              ...
              <div className="card p-5 flex flex-col items-center justify-center text-center">
                ...
                <span className="text-sm text-white/50 mt-1">Karya Terverifikasi</span>
              </div>
            </>
          )}
          ...
          {user?.role === 'pengunjung' && (
            <div className="card p-5 col-span-2 flex flex-col items-center justify-center text-center gap-3">
              ...
              <p className="text-white/70 text-sm">Daftar sebagai mahasiswa untuk bisa mengupload karya!</p>
              <Link to="/register" className="btn-primary text-sm px-6 py-2">Daftar Sebagai Mahasiswa</Link>
            </div>
          )}
        </div>
        ...
        {user?.role === 'mahasiswa' && (
          <div>
            <div className="flex items-center justify-between mb-5">
              ...
              <Link to="/upload" className="text-primary-400 text-sm hover:text-primary-300">+ Upload Baru</Link>
            </div>

            {loading ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                {[...Array(3)].map((_, i) => <div key={i} className="card aspect-[4/3] animate-pulse" />)}
              </div>
            ) : myArtworks.length === 0 ? (
              <div className="card p-12 text-center">
                <span className="text-5xl mb-4 block">🖼️</span>
                <p className="text-white/50 mb-4">Belum ada karya yang diupload</p>
                <Link to="/upload" className="btn-primary px-6">Upload Karya Pertama</Link>
              </div>
            ) : (
                ...
                {myArtworks.map(a => (
                  <div key={a.id} className="card p-4 flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <Link to={`/artworks/${a.id}`} className="btn-secondary text-xs py-1.5 px-3">
                        Lihat
                      </Link>
                      <button onClick={() => handleDelete(a.id)} className="btn-danger text-xs py-1.5 px-3">
                        Hapus
                      </button>
                    </div>
                  </div>
